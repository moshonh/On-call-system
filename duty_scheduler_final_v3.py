...
# PAGE CONFIG
# ══════════════════════════════════════════════════════
st.set_page_config(page_title="ניהול כוננויות - גרסה סופית בהחלט", page_icon="🏥", layout="wide")
...
def build_schedule(
    ...
    def get_score(assignment):
        ...
        for (d, s), doc in assignment.items():
            if s in (1, 2): 
                active_counts[doc] += 1
                # Check if it's a weekend/holiday block for active duties
                # We only count the start of the block to avoid double counting days
                is_weekend = False
                for blk in blocks:
        ...
        weekend_penalty = sum(max(0, count - 1) * 500 for count in weekend_active_counts.values())
        ...
        return (a_diff * 100) + (p_diff * 100) + (t_diff * 100) + weekend_penalty + pref_score

    current_assignment = None
    for _ in range(200): # Try multiple times to get a valid start
    ...
    for i in range(max_iterations):
        ...
        if current_score < 3.1: # Perfect balance and no weekend penalty
            break
...
st.markdown('<div class="main-title">🏥 ניהול כוננויות - גרסה סופית בהחלט</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">הגבלות טורים ספציפיות · מקסימום סופ"ש אחד בחודש לכוננות אקטיבית</div>', unsafe_allow_html=True)
...
with st.expander("🚫 חסימות והעדפות"):
    ...
    for i, doc in enumerate(doctors):
        with cols[i % 2]:
            ...
            # Default rules based on user request
