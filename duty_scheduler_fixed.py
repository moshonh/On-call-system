...
def build_schedule(
    ...
    max_coupled_per_doc: int = 1,   # כל רופא עושה לכל היותר N בלוקי-צמד בטור3
) -> tuple[dict, dict, dict, dict]:
    """
    אלגוריתם שלושה שלבים:

    שלב א' – טור 3 (פסיבי):
      • מחלק קודם כל את כל בלוקי טור 3.
      • ספירה לפי שיבוצים (לא ימים) → הפרש ≤1 מובטח.
      • כל רופא עושה לכל היותר max_coupled_per_doc בלוקי-צמד
        (שישי+שבת / חג). מגביל עומס לא-שוויוני.

    שלב ב' – טורים 1+2 (כבד):
      • בוחר לפי total = cnt1+cnt2+cnt3 בזמן אמת.
      • שומר: לא אותו רופא בשני טורים שונים באותו בלוק.

    ערבות: הפרש שיבוצי טור3 ≤1, הפרש שיבוצים סה"כ ≤2.
    """
    blocks = build_coupled_blocks(year, month, holidays)
    assignment: dict = {}

    # cnt[s] = ספירת שיבוצים (בלוקים, לא ימים) – מנגנון השוויון
    cnt: dict = {1: defaultdict(int), 2: defaultdict(int), 3: defaultdict(int)}
    # coupled_cnt = כמה בלוקי-צמד קיבל כל רופא (לטור 3)
    coupled_cnt: dict = defaultdict(int)
    # days_disp = ימים בפועל – לתצוגה בלבד
    days_disp: dict = {1: defaultdict(int), 2: defaultdict(int), 3: defaultdict(int)}
    pref_granted: dict = defaultdict(int)

    cnt_list = [cnt[1], cnt[2], cnt[3]]  # reference list – מתעדכן אוטומטית
    ...
    for blk in blocks:
        ...
        # הגבלת בלוקי-צמד לרופא (שישי+שבת / חג)
        if is_coupled and len(cands) > 1:
            not_overloaded = [d for d in cands
                              if coupled_cnt.get(d, 0) < max_coupled_per_doc]
            if not_overloaded:
                cands = not_overloaded

        chosen = pick_fair(cands, cnt[3], cnt_list, personal_prefs, days)
        if not chosen:
            chosen = random.choice(doctors)

        for day in days:
            assignment[(day, 3)] = chosen
        cnt[3][chosen]       += 1        # ← שיבוץ אחד (לא w)
        days_disp[3][chosen] += w        # ← ימים בפועל (לתצוגה)
        if is_coupled:
            coupled_cnt[chosen] += 1
        if block_has_preference(chosen, days, personal_prefs):
            pref_granted[chosen] += 1

    # ─────────────────────────────────────────
    # שלב ב': טורים 1+2 – כל הבלוקים
    # ─────────────────────────────────────────
    for blk in blocks:
        days = blk["days"]
        w    = len(days)
        ...
        for shift in [1, 2]:
            ...
            chosen = pick_fair(cands, cnt[shift], cnt_list, personal_prefs, days)
            if not chosen:
                chosen = random.choice(doctors)
...
st.markdown(
    '<div class="sub-title">'
    'שני סלים · חוק צמד (שישי+שבת, ערב-חג+חג) · הפרש ≤1 · ייצוא CSV ו-Excel'
    '</div>',
    unsafe_allow_html=True,
)
...
with st.expander("⚙️ הגדרות בסיסיות", expanded=True):
    ...
    st.markdown(
        '<div class="info-box">⚙️ הגבלת בלוקי שישי+שבת / חג לרופא בטור פסיבי '
        '(ברירת מחדל: 1 בלוק צמד לכל רופא)</div>',
        unsafe_allow_html=True,
    )
    max_coupled = st.slider(
        "מקסימום בלוקי-צמד (שישי+שבת/חג) לרופא בטור 3",
        min_value=1, max_value=5, value=1,
        help="ערך 1 = כל רופא עושה לכל היותר שישי+שבת/חג אחד בטור הפסיבי",
    )
...
if run_btn:
    ...
    assignment, heavy, light, pref_granted = build_schedule(
        ...
        max_coupled_per_doc=max_coupled,
    )
