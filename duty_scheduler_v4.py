...
st.markdown('<div class="sub-title">שני סלים · חוק צמד · חסימות קשות · העדפות רכות · ייצוא Excel</div>',
            unsafe_allow_html=True)
...
def pick_doctor(candidates: list[str], pool_count: dict,
                personal_prefs: dict, block_days: list[int],
                fairness_tolerance: int) -> str:
    """
    בוחר רופא מתוך המועמדים תוך שמירה על שוויון.

    לוגיקת העדפות רכות (soft preferences):
    1. מזהה מועמדים שביקשו את הימים האלה (preferred).
    2. מחשב מי מהם לא חורג מהממוצע+tolerance.
    3. אם יש מועמד מועדף שעדיין "בטווח" – בוחר ממנו.
    4. אחרת – חוזר לבחירה הוגנת רגילה (הכי פחות כוננויות).

    כך העדפה לא יכולה לייצר פערים גדולים, אבל מקוזז
    לפי הסדר הטבעי של הזמנות.
    """
    if not candidates:
        return ""

    # ── שלב א': מציאת הממוצע + מינימום עדכני ──
    counts = [pool_count.get(d, 0) for d in candidates]
    min_c  = min(counts)
    avg_c  = sum(counts) / len(counts)
    # רצפה: לא נתן לאחד לחרוג ב-tolerance מהממוצע הנוכחי
    ceiling = avg_c + fairness_tolerance

    # ── שלב ב': מועמדים מועדפים שעדיין בטווח ──
    preferred_in_range = [
        d for d in candidates
        if block_has_preference(d, block_days, personal_prefs)
        and pool_count.get(d, 0) <= ceiling
    ]

    if preferred_in_range:
        # מתוך המועדפים – עדיף מי שיש לו הכי פחות כוננויות
        min_pref = min(pool_count.get(d, 0) for d in preferred_in_range)
        tied_pref = [d for d in preferred_in_range
                     if pool_count.get(d, 0) == min_pref]
        return random.choice(tied_pref)

    # ── שלב ג': ברירת מחדל – הכי פחות כוננויות (הגינות) ──
...
def build_schedule(
    ...
    מחזיר:
      ...
      heavy_count  : dict[doctor] -> int
      light_count  : dict[doctor] -> int
    ...
    for blk in blocks:
        ...
        for shift in [1, 2, 3]:
            ...
            # מניעת כפילות בבלוק (אותו רופא בשני טורים)
            already_in_block = {
            ...
            # ── בחירה עם תמיכה ב-soft preferences ──
            chosen = pick_doctor(
                cands, pool_count,
                personal_prefs, days,
                fairness_tolerance,
...
with st.expander("⭐ העדפות רכות – ימים שהרופא מעוניין בהם"):
    ...
    tol_col, _ = st.columns([1, 2])
    with tol_col:
        fairness_tolerance = st.slider(
            "סף הוגנות (מקסימום פער מותר בין רופאים)",
            min_value=1, max_value=5, value=2,
            help="1 = העדפות מאוד מוגבלות, 5 = העדפות גמישות יותר. ברירת מחדל 2 מאפשרת העדפות סבירות מבלי לפגוע בשוויון.",
        )
