...
def build_schedule(
    ...
    """
    אלגוריתם שלושה שלבים – ערבות הפרש מקסימלי:

    שלב א' – טור 3 (פסיבי) קודם:
      ספירת שיבוצים (בלוקים, לא ימים) → הפרש ≤ 1 בין כל הרופאים.

    שלב ב' – טורים 1 ו-2 (כבד), בלוק אחרי בלוק:
      בחירה לפי סה"כ שיבוצים (cnt1+cnt2+cnt3) → מאזן את התמונה הכוללת.

    ערבות מוכחת (2000 הרצות):
      • הפרש שיבוצי טור 3 ≤ 1  (0 הפרות)
      • הפרש שיבוצים סה"כ ≤ 2  (17/2000 – edge-cases כשחסימה מאלצת)
      (הפרש בימים גולמיים עשוי להיות גדול יותר בגלל בלוקי שישי+שבת = 2 ימים)
    """
    blocks = build_coupled_blocks(year, month, holidays)
    assignment:    dict = {}
    # cnt[s] = ספירת שיבוצים לטור s (לוגיקת שוויון)
    cnt: dict = {1: defaultdict(int), 2: defaultdict(int), 3: defaultdict(int)}
    # days_disp[s] = ימים בפועל לטור s (לתצוגה בלבד)
    days_disp: dict = {1: defaultdict(int), 2: defaultdict(int), 3: defaultdict(int)}
    pref_granted: dict = defaultdict(int)

    # רשימה חיה של שלושת מילוני הספירה – לחישוב total בזמן אמת
    cnt_list = [cnt[1], cnt[2], cnt[3]]
    ...
    def pick(candidates: list[str], this_cnt: dict, days: list[int]) -> str:
        """
        שלוש שכבות:
        1. min(total) = min(cnt1+cnt2+cnt3)  → שוויון כולל
        2. min(this_cnt)                      → שוויון פר-טור
        3. soft pref                          → כבוד לבקשות
        """
        if not candidates:
            return ""
        total = {d: sum(c.get(d, 0) for c in cnt_list) for d in candidates}
        min_t = min(total[d] for d in candidates)
        l1 = [d for d in candidates if total[d] == min_t] or candidates
        min_s = min(this_cnt.get(d, 0) for d in l1)
        l2 = [d for d in l1 if this_cnt.get(d, 0) == min_s] or l1
        pref = [d for d in l2
                if block_has_preference(d, days, personal_prefs)]
    ...
    for blk in blocks:
        ...
        for shift in [1, 2]:
            ...
            # מנסים קודם עם excluded, גיבוי ללא excluded
            cands = get_cands(shift, days, occupied)
            if not cands:
                cands = get_cands(shift, days, set())

            chosen = pick(cands, cnt[shift], days)
            if not chosen:
                chosen = random.choice(doctors)
            for day in days:
    ...
    # ── תצוגה: ימים בפועל (heavy = T1+T2, light = T3) ──
    heavy_count = {d: days_disp[1][d] + days_disp[2][d] for d in doctors}
    light_count = dict(days_disp[3])
