...
# PAGE CONFIG
# ══════════════════════════════════════════════════════
st.set_page_config(page_title="ניהול כוננויות - גרסה חסינה", page_icon="🏥", layout="wide")
...
def build_schedule(
    ...
    # Initial random valid assignment
    def get_initial():
        assignment = {}
        for blk in blocks:
            days = blk["days"]
    ...
    def get_score(assignment):
        ...
        # Count blocks for active, days for passive
        # Actually, user wants days to be equal too, but blocks are the constraint.
        # Let's count days for everything to be safe.
        for (d, s), doc in assignment.items():
            if s in (1, 2): active_counts[doc] += 1
            else: passive_counts[doc] += 1
            total_counts[doc] += 1
        ...
        # Preference score (negative because we want to minimize score)
        pref_score = 0
        for (d, s), doc in assignment.items():
            if d in personal_prefs.get(doc, []):
                pref_score -= 0.1
                
        return (a_diff * 100) + (p_diff * 100) + (t_diff * 100) + pref_score

    current_assignment = None
    for _ in range(100): # Try multiple times to get a valid start
    ...
    # Hill Climbing with random restarts/jumps
    for i in range(max_iterations):
        new_assignment = copy.deepcopy(current_assignment)
        
        # Randomly pick a day and a shift to swap or change
        if random.random() < 0.5:
            # Change a block (Active)
        ...
        # Acceptance criteria (Metropolis-like or simple hill climbing)
        if new_score <= current_score:
            current_assignment = new_assignment
            current_score = new_score
            
        # If we reached a perfect balance, we can stop early
        if current_score < 3.1: # 1+1+1 diffs
            break
...
st.markdown('<div class="main-title">🏥 ניהול כוננויות - גרסה חסינה</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">מנוע אופטימיזציה מתקדם להתמודדות עם חסימות רבות</div>', unsafe_allow_html=True)
