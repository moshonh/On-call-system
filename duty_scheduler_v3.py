...
# PAGE CONFIG
# ══════════════════════════════════════════════════════
st.set_page_config(page_title="ניהול כוננויות - גרסה סופית משופרת", page_icon="🏥", layout="wide")
...
def build_coupled_blocks(year: int, month: int, holidays: set) -> list[dict]:
    ...
    while d <= num_days:
        ...
        if dtype in ("fri", "hol", "hol_eve"):
            ...
            # Special case: if a holiday eve is followed by a weekend, it's already covered by the loop above
            # because 'fri' starts a block and 'sat' continues it. 
            # If 'hol_eve' is Thursday and 'fri' is next, the loop continues.
...
def build_schedule(
    ...
    def get_score(assignment):
        ...
        # Penalty for more than 1 weekend/holiday block in active duties
        weekend_penalty = sum(max(0, count - 1) * 1000 for count in weekend_active_counts.values())
        
        # Preference score - Higher weight for active duties (1 and 2)
        pref_score = 0
        for (d, s), doc in assignment.items():
            if d in personal_prefs.get(doc, []):
                if s in (1, 2):
                    pref_score -= 5.0  # High priority for active
                else:
                    pref_score -= 0.1  # Low priority for passive
                
        return (a_diff * 200) + (p_diff * 200) + (t_diff * 200) + weekend_penalty + pref_score
    ...
    for i in range(max_iterations):
        ...
        if current_score < -50: # Arbitrary threshold for "good enough" with preferences
            # But we still want to keep iterating to find better balance if possible
            pass
...
st.markdown('<div class="main-title">🏥 ניהול כוננויות - גרסה סופית משופרת</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">תעדוף העדפות לכוננות אקטיבית · חוק צמד מורחב (ערבי חג צמודים לסופ"ש)</div>', unsafe_allow_html=True)
